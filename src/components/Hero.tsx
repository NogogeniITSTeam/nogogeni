...
function Hero() {
  ...
  return (
    <section className="relative bg-nogogeni-black mt-16 desktop:mt-24">
      <Carousel setApi={setCarouselApi} plugins={[Autoplay({ delay: 2000 })]}>
        <CarouselContent>
          {heroImages.map((img) => (
            <CarouselItem
              key={img.alt}
              className="relative h-72 tablet:h-[512px] desktop:h-[calc(100vh-96px)]"
            >
              <Image
                fill
                src={img.path}
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-3 desktop:gap-4">
